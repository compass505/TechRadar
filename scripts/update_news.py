...
- 5: 重大リスクが現実化している、または短期的に広範な被害・強制対応・制度変更を起こしうる例外的ニュース

重要度5は絞ってください:
- 単に「AI」「セキュリティ」「法規制」に関わるだけなら4以下にする
- レポート、解説、新機能、実証実験、限定提供、研究発表は原則4以下にする
- 5にするのは、悪用中/PoC公開済みの重大脆弱性、重要インフラへの緊急要請、国政選挙や公衆安全への直接影響、大規模漏えい・停止などに限る
- 4と5で迷う場合は必ず4にする
